...
电机接口、D24A 输出、软件状态和屏幕菜单统一使用 A/B/C/D 直接对应。A/C 与 B/D 两侧电机采用镜像安装，因此车辆前进时两侧电机轴的物理旋转方向相反；软件中的正目标转速统一表示车辆前进。
...
- 上电后默认处于 `READY`，四路目标速度为 0 RPM；进入 MOTOR 菜单并把 RUN 设为 ON 后才开始输出，PB21 急停和故障锁存始终优先阻止启动。
...
MOTOR sets a common target from 0 to 600 RPM in 10 RPM steps and provides independent A/B/C/D channel enable switches. A disabled channel is commanded to zero duty and its PID state is reset. BUZZER controls the active buzzer on PB27 with OFF, CONT, SLOW, and FAST gating modes plus 10% to 100% gate volume; it does not change the buzzer's internal tone frequency. LED controls three serial WS2812B devices on PA29 with count 0 to 3, brightness 0% to 100%, and independent LED1/LED2/LED3 colors. PB22 remains the onboard LED.
